...
import "@/app/dashboard/organizer-dashboard.css";
...
// Upcoming and Past are two routes over one board: /dashboard/organizer/[id]
// and /dashboard/organizer/[id]/past-events. They share the filter bar and
// every modal, so they are one component told which half to show rather than
// two copies that would drift. Each asks the server for ONLY its half
// (`?scope=`), so landing on the dashboard never downloads the organiser's
// whole history — and each half arrives a page at a time.
export type GamesView = "upcoming" | "past";

// How many games the server sends per page. Both views are paged server-side,
// filters included — see getOrganiserGames.
const PAGE_SIZE = 20;
...
// Each view has its own natural reading order: Upcoming starts with the next
// fixture, Past starts with the one just played — that most recent game is what
// an organiser opens the page for (attendance, ratings, settlement).
const DEFAULT_SORT: Record<GamesView, string> = { upcoming: 'date-asc', past: 'date-desc' };

// Paged fetches can overlap by a row when games move between pages (a game
// finishing pushes everything down one), so a merge keeps the first copy.
const dedupeById = (list: any[]) => {
  const seen = new Set<string>();
  return list.filter((g) => (seen.has(g._id) ? false : (seen.add(g._id), true)));
};

export default function OrganiserGamesBoard({ view }: { view: GamesView }) {
  ...
  // Both halves' sizes, for the tab badges — the other half's games are never
  // fetched, so its count has to come from the server.
  const [scopeCounts, setScopeCounts] = useState<{ upcoming: number; past: number } | null>(null);
  ...
  // Bumped by every fetch that REPLACES or EXTENDS the list. A response whose
  // number is no longer current was overtaken (the filters changed, or a page
  // was appended) and is dropped rather than painted over newer data.
  ...
  // ── Server-side paging ────────────────────────────────────────────────────
  // Both views arrive a page at a time, so the filter bar is applied by the
  // server too — in the browser a search would only look through the pages
  ...
  const listQuery = new URLSearchParams({ q: debouncedSearch.trim(), status: filterStatus, format: filterFormat, sort: sortBy }).toString();
  const listQueryRef   = useRef(listQuery);
  const pagesLoadedRef = useRef(1);
  const [paging, setPaging]     = useState<{ hasMore: boolean; total: number } | null>(null);
  const [scopeFormats, setScopeFormats]   = useState<string[] | null>(null);
  const [refiltering, setRefiltering] = useState(false);
  const [loadingMore, setLoadingMore]   = useState(false);
  const loadingMoreRef = useRef(false);
  // Games the decision pop-up may need, sent by the server apart from the page
  // (null = an older backend that sends none; the pop-up then reads the page).
  const [alertGames, setAlertGames] = useState<any[] | null>(null);
  // An in-place patch (confirm, switch, withdraw…) must reach the pop-up's
  // copy as well, or a game confirmed from the pop-up but not on the loaded
  // page would keep asking to be confirmed until the next refresh.
  const patchGames = useCallback((fn: (g: any) => any) => {
    setGames((prev) => prev.map(fn));
    setAlertGames((prev) => (prev ? prev.map(fn) : prev));
  }, []);
  ...
  const scopeUrl = useCallback((page: number) =>
    `/api/v1/games/organisers/my-games?scope=${view}&page=${page}&limit=${PAGE_SIZE}&${listQueryRef.current}`,
  [view]);

  // Replaces the list — every page already loaded, in
  // parallel, so a background refresh never snatches back what the organiser
  // had scrolled to. `force` is for a filter change, which must not be skipped
  // just because a background poll happens to be in flight.
  const fetchGames = useCallback(async (options?: { silent?: boolean; force?: boolean }) => {
    const silent = options?.silent === true;
    if (isFetchingGamesRef.current && !options?.force) return;
    ...
    try { 
      ...
      const responses = await Promise.all(
        Array.from({ length: pages }, (_, i) =>
          fetchWithLocalFallback(buildApiUrl(scopeUrl(i + 1)), {
            headers: {
      ...
      const failed = pagesData.find((d) => !d.success);
      const data = failed ?? pagesData[pagesData.length - 1];
      
      if (data.success) {
        const nextGames: any[] = dedupeById(pagesData.flatMap((d) => d.data || []));
        setGames(nextGames);
        setScopeCounts(data.counts ?? null);
        setPaging({ hasMore: !!data.hasMore, total: data.total ?? nextGames.length });
        setScopeFormats(Array.isArray(data.formats) ? data.formats : null);
        setAlertGames(Array.isArray(data.alerts) ? data.alerts : null);
        setFetchError(null);  

        // "lastEvent" pre-fills the create form with the most recently CREATED
        // game. This fetch holds only one half of the organiser's games, so only
        // overwrite the stored one with a newer game — otherwise opening Past
        // Events would swap it for an old fixture.
        if(nextGames.length > 0) 
         {
          const createdOf = (g: any) => new Date(g?.createdAt || g?.scheduledAt || 0).getTime();
          const lastEvent = [...nextGames].sort((a, b) => createdOf(b) - createdOf(a))[0];
          let stored: any = null;
          try { stored = JSON.parse(localStorage.getItem("lastEvent") || "null"); } catch {}
          if (!stored || createdOf(lastEvent) >= createdOf(stored)) {
      ...
      if (!silent) setLoading(false);
      if (seq === fetchSeqRef.current) setRefiltering(false);
      isFetchingGamesRef.current = false;
    }
  }, [fetchWithLocalFallback, router, view, scopeUrl, setGames]);

  // Appends the next page.
  const hasMorePage = !!paging?.hasMore;
  const loadMore = useCallback(async () => {
    if (loadingMoreRef.current || !hasMorePage) return;
    const { token } = getSession();
    if (!token) { clearSession(); router.replace("/login?role=organiser"); return; }
    loadingMoreRef.current = true;
    setLoadingMore(true);
    // Its own sequence number: a background poll still in flight for the OLD
    // page count would otherwise land after this and drop the page just added.
    const seq = ++fetchSeqRef.current;
    const nextPage = pagesLoadedRef.current + 1;
    try {
      const res  = await fetchWithLocalFallback(buildApiUrl(scopeUrl(nextPage)), { headers: { Authorization: `Bearer ${token}` } });
      const data = await res.json();
      if (seq !== fetchSeqRef.current) return;
      if (!res.ok || !data.success) { showToast("error", data.message || "Couldn't load more events"); return; }
      pagesLoadedRef.current = nextPage;
      setGames((prev) => dedupeById([...prev, ...(data.data || [])]));
      setScopeCounts(data.counts ?? null);
      setPaging({ hasMore: !!data.hasMore, total: data.total ?? 0 });
      if (Array.isArray(data.alerts)) setAlertGames(data.alerts);
    } catch {
      showToast("error", "Couldn't load more events. Please try again.");
    } finally {
      loadingMoreRef.current = false;
      setLoadingMore(false);
    }
  }, [hasMorePage, router, fetchWithLocalFallback, scopeUrl, showToast, setGames]);

  // A filter change is a different list: back to page one, fetched now.
  useEffect(() => {
    if (listQueryRef.current === listQuery) return;
    listQueryRef.current = listQuery;
    pagesLoadedRef.current = 1;
    setRefiltering(true);
    fetchGames({ silent: true, force: true });
  }, [listQuery, fetchGames]);

  // Scrolling near the end of the list reveals the next page. A callback ref,
  // so the observer re-attaches when the sentinel actually mounts.
  const [sentinel, sentinelRef] = useState<HTMLElement | null>(null);
  useEffect(() => {
    if (!sentinel || !hasMorePage || typeof IntersectionObserver === "undefined") return;
  ...
  const handleConfirmGame = async (gameId: string) => {
    ...
    try {
      ...
      if (data.data) {
        ...
        patchGames((g) => g._id === updated._id ? updated : g);
        setSelectedGame((prev: any) => prev?._id === updated._id ? updated : prev);
      } else {
        patchGames((g) => g._id === gameId ? { ...g, status: "confirmed" } : g);
      }
    } catch (err) {
    ...
    const doWithdraw = async () => {
      ...
      try {
        ...
        if (data.data) {
          ...
          patchGames((g) => g._id === gameId ? { ...g, organiserIsPlaying: false } : g);
        }
      } catch (err) {
  ...
  // The server already sent only this view's half, filtered and sorted. The
  // split is re-applied here so a game that kicks off while the page is open
  // leaves Upcoming at once rather than on the next refresh.
  const now = new Date();
  ...
  const allFormats = scopeFormats
    ? scopeFormats
    : [...new Set(games.map((g: any) => g.format).filter(Boolean))] as string[];

  // Both lists are filtered and sorted by the server; re-filtering here would
  // only disagree with it about the pages that are not loaded.
  const filteredUpcoming = upcomingGames;
  const filteredPast     = pastGames;
  const shownGames       = view === 'upcoming' ? filteredUpcoming : filteredPast;
  const hasActiveFilters = !!(searchQuery || filterStatus !== 'all' || filterFormat !== 'all' || sortBy !== defaultSort);
  const listTotal        = paging?.total ?? shownGames.length;

  // What the decision pop-up looks at: the server's alert games, with any that
  // are on screen taken from the page (the fresher copy after a patch).
  const loadedById = new Map(games.map((g) => [g._id, g]));
  const lifecycleGames = alertGames ? alertGames.map((a) => loadedById.get(a._id) ?? a) : games;

  const clearFilters = () => { setSearchQuery(''); setFilterStatus('all'); setFilterFormat('all'); setSortBy(defaultSort); };
    ...
    <div className="organizer-dashboard-container">
      ...
      <div className="dashboard-header-section">
        <div className="header-left">
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <h1 className="dashboard-title">{view === 'past' ? 'Past Events' : 'Organizer Dashboard'}</h1>
            <span className="live-badge"><span className="live-dot" />Live</span>
          </div>
          <p className="dashboard-subtitle">
            {view === 'past'
              ? 'Games that have been played or cancelled — attendance, ratings and team history'
      ...
      <div className="tabs-section">
        <div className="tab-navigation">
          <Link
            href={`/dashboard/organizer/${organiserId}`}
          ...
          >
            ...
            <span className="tab-badge">{scopeCounts?.upcoming ?? upcomingGames.length}</span>
          </Link>
          <Link
            href={`/dashboard/organizer/${organiserId}/past-events`}
          ...
          >
            ...
            <span className="tab-badge">{scopeCounts?.past ?? pastGames.length}</span>
          </Link>
        </div>
      </div>
      ...
      {!loading && games.length > 0 && (
        <div className="filter-bar">
          ...
          <span className="filter-result-count">
            {listTotal}
            {' '}/{' '}
            {(view === 'upcoming' ? scopeCounts?.upcoming : scopeCounts?.past) ?? listTotal} events
          </span>
        </div>
      )}
      ...
      {/* <div className="table-section"> */}
        {loading ? (
          ...
          filteredUpcoming.length === 0 ? (
            ...
            <>
            <div className="games-list" style={refiltering ? { opacity: 0.55, transition: "opacity .15s" } : undefined} aria-busy={refiltering}>
            ...
            {/* Scrolling to here fetches the next page. The button is the
                fallback for keyboard users and for anything without
                IntersectionObserver. */}
            {hasMorePage && (
              <div className="kk-load-more" ref={sentinelRef}>
                <button type="button" className="kk-load-more-btn" onClick={loadMore} disabled={loadingMore}>
                  {loadingMore ? "Loading…" : "Show more events"}
                </button>
                <span className="kk-load-more-count" aria-live="polite">
                  {shownGames.length} of {listTotal}
                </span>
              </div>
            )}
            </>
      ...
      {showPlayersModal && selectedGame && (
        <PlayerDetailsModal
          ...
          onGameUpdate={(updated) => {
            patchGames((g) => g._id === updated._id ? { ...g, ...updated } : g);
